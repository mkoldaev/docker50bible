#include <map>
#include <string>
class Bible48
{
	struct af1	{ int val; const char *msg; };
	struct af2	{ int val; const char *msg; };
	struct af3	{ int val; const char *msg; };
	struct af4	{ int val; const char *msg; };
	struct af5	{ int val; const char *msg; };
	struct af6	{ int val; const char *msg; };
public:
static void view1() {
struct af1 poems[] = {
	{1, "1 Paulus, 'n apostel nie vanweë mense of deur 'n mens nie, maar deur Jesus Christus en God die Vader wat Hom uit die dode opgewek het,"},
	{2, "2 en al die broeders wat by my is--aan die gemeentes van Galásië:"},
	{3, "3 Genade vir julle en vrede van God die Vader en onse Here Jesus Christus,"},
	{4, "4 wat Homself gegee het vir ons sondes, sodat Hy ons kan uitred uit die teenwoordige bose wêreld, volgens die wil van onse God en Vader,"},
	{5, "5 aan wie die heerlikheid toekom in alle ewigheid! Amen."},
	{6, "6 Ek verwonder my dat julle so gou afvallig word van hom wat julle deur die genade van Christus geroep het, na 'n ander evangelie toe,"},
	{7, "7 terwyl daar geen ander is nie; behalwe dat daar sommige mense is wat julle in die war bring en die evangelie van Christus wil verdraai."},
	{8, "8 Maar al sou ons of 'n engel uit die hemel julle 'n evangelie verkondig in stryd met die wat ons julle verkondig het, laat hom 'n vervloeking wees!"},
	{9, "9 Soos ons vantevore gesê het, sê ek nou ook weer: As iemand julle 'n evangelie verkondig in stryd met die wat julle ontvang het, laat hom 'n vervloeking wees!"},
	{10, "10 Soek ek dan nou die guns van mense, of van God? Of probeer ek om mense te behaag? As ek nog mense behaag, dan sou ek nie 'n dienskneg van Christus wees nie."},
	{11, "11 Maar ek maak julle bekend, broeders, dat die evangelie wat deur my verkondig is, nie na die mens is nie."},
	{12, "12 Want ek het dit ook nie van 'n mens ontvang of geleer nie, maar deur 'n openbaring van Jesus Christus."},
	{13, "13 Want julle het gehoor van my lewenswandel vroeër in die Jodedom, dat ek die gemeente van God uitermate vervolg en dit verwoes het;"},
	{14, "14 en dat ek in die Jodedom vooruitgegaan het bo baie van my leeftyd onder my stamgenote, terwyl ek veral 'n yweraar was vir die oorleweringe van my voorvaders."},
	{15, "15 Maar toe dit God behaag het, wat my afgesonder het van die skoot van my moeder af en deur sy genade geroep het,"},
	{16, "16 om sy Seun in my te openbaar, sodat ek Hom onder die heidene sou verkondig, het ek dadelik nie vlees en bloed geraadpleeg nie,"},
	{17, "17 en ook nie na Jerusalem opgegaan na die wat voor my apostels was nie, maar na Arabië vertrek en weer teruggekom na Damaskus."},
	{18, "18 Vervolgens, drie jaar daarna, het ek na Jerusalem opgegaan om met Petrus kennis te maak en vyftien dae by hom gebly;"},
	{19, "19 en ek het niemand anders van die apostels gesien nie, behalwe Jakobus, die broer van die Here."},
	{20, "20 En wat ek julle skryf--voor die aangesig van God, ek lieg nie!"},
	{21, "21 Daarna het ek gekom in die streke van Sírië en Cilícië;"},
	{22, "22 en ek was persoonlik onbekend by die Christelike gemeentes van Judéa;"},
	{23, "23 hulle het net gehoor: hy wat ons vroeër vervolg het, verkondig nou die geloof wat hy vroeër besig was om uit te roei."},
	{24, "24 En hulle het God verheerlik in my."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
static void view2() {
struct af2 poems[] = {
	{1, "1 Daarna, veertien jaar later, het ek weer na Jerusalem opgegaan met Barnabas, en Titus ook saamgeneem."},
	{2, "2 Maar ek het opgegaan op grond van 'n openbaring en hulle die evangelie voorgelê wat ek onder die heidene verkondig; en afsonderlik aan die wat in aansien was, dat ek nie miskien tevergeefs sou loop of geloop het nie."},
	{3, "3 Maar selfs is Titus, wat by my was, nie gedwing om besny te word nie, alhoewel hy 'n Griek is;"},
	{4, "4 en dit vanweë die ingesmokkelde valse broeders wat ingesluip het om ons vryheid te bespied wat ons in Christus Jesus het, sodat hulle ons in diensbaarheid kon bring;"},
	{5, "5 aan wie ons selfs nie 'n oomblik in onderworpenheid toegegee het nie, sodat die waarheid van die evangelie by julle bestendig sou bly."},
	{6, "6 Maar van die wat in aansien was--wat hulle vroeër was, maak by my geen verskil nie; God neem die persoon van die mens nie aan nie--hulle dan wat in aansien was, het my niks meer opgelê nie."},
	{7, "7 Maar, inteendeel, toe hulle sien dat aan my die evangelie vir die onbesnedenes toevertrou is, net soos aan Petrus dié vir die besnedenes--"},
	{8, "8 want Hy wat Petrus bekragtig het met die oog op die apostelskap vir die besnedenes, het my ook bekragtig met die oog op die heidene--"},
	{9, "9 en toe hulle merk die genade wat aan my gegee is, het Jakobus en Céfas en Johannes, wat as pilare geag is, aan my en Barnabas die regterhand van gemeenskap gegee, sodat ons na die heidene en hulle na die besnedenes kon gaan;"},
	{10, "10 alleen moes ons aan die armes dink, wat ek my juis ook beywer het om te doen."},
	{11, "11 Maar toe Petrus in Antiochíë gekom het, het ek hom openlik teëgestaan, omdat hy veroordeeld gestaan het."},
	{12, "12 Want voordat sommige van Jakobus af gekom het, was hy gewoond om saam met die heidene te eet; maar ná hulle koms het hy hom teruggetrek en hom eenkant gehou uit vrees vir die wat uit die besnydenis is."},
	{13, "13 En saam met hom het ook die ander Jode geveins, sodat selfs Barnabas hom laat meevoer het deur hulle geveins."},
	{14, "14 Maar toe ek sien dat hulle nie reguit loop volgens die waarheid van die evangelie nie, het ek vir Petrus in teenwoordigheid van almal gesê: As jy wat 'n Jood is, soos 'n heiden lewe en nie soos 'n Jood nie, waarom dwing jy die heidene om soos Jode te lewe?"},
	{15, "15 Ons wat van nature Jode en nie sondaars uit die heidene is nie,"},
	{16, "16 terwyl ons weet dat die mens nie geregverdig word uit die werke van die wet nie, maar alleen deur die geloof in Jesus Christus, selfs ons het in Christus Jesus geglo, sodat ons geregverdig kan word uit die geloof in Christus en nie uit die werke van die wet nie; omdat uit die werke van die wet geen vlees geregverdig sal word nie."},
	{17, "17 Maar as ons wat streef om in Christus geregverdig te word, self ook sondaars bevind is, is Christus dan 'n dienaar van die sonde? Nee, stellig nie!"},
	{18, "18 Want as ek weer opbou wat ek afgebreek het, dan betoon ek myself 'n oortreder;"},
	{19, "19 want deur die wet is ek vir die wet dood om vir God te lewe."},
	{20, "20 Ek is met Christus gekruisig, en ék leef nie meer nie, maar Christus leef in my. En wat ek nou in die vlees lewe, leef ek deur die geloof in die Seun van God wat my liefgehad het en Homself vir my oorgegee het."},
	{21, "21 Ek verwerp nie die genade van God nie; want as daar geregtigheid deur die wet is, dan het Christus tevergeefs gesterwe."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
static void view3() {
struct af3 poems[] = {
	{1, "1 o, Onverstandige Galásiërs, wie het julle betower om die waarheid nie gehoorsaam te wees nie, julle voor wie se oë Jesus Christus afgeskilder is as onder julle gekruisig?"},
	{2, "2 Dit alleen wil ek van julle weet: het julle die Gees ontvang uit die werke van die wet of uit die prediking van die geloof?"},
	{3, "3 Is julle so onverstandig? Nadat julle met die Gees begin het, eindig julle nou met die vlees?"},
	{4, "4 Het julle verniet so baie gely? As dit maar verniet was!"},
	{5, "5 Hy wat julle dan die Gees verleen en kragte onder julle werk, doen Hy dit uit die werke van die wet of uit die prediking van die geloof?"},
	{6, "6 Net soos Abraham in God geglo het, en dit is hom tot geregtigheid gereken."},
	{7, "7 Julle verstaan dan dat die wat uit die geloof is, hulle is kinders van Abraham."},
	{8, "8 En die Skrif wat vooruit gesien het dat God die heidene uit die geloof sou regverdig, het vooraf aan Abraham die evangelie verkondig met die woorde: In jou sal al die volke geseën word."},
	{9, "9 Sodat die wat uit die geloof is, geseën word saam met die gelowige Abraham."},
	{10, "10 Want almal wat uit die werke van die wet is, is onder die vloek; want daar is geskrywe: Vervloek is elkeen wat nie bly in alles wat geskryf is in die boek van die wet om dit te doen nie."},
	{11, "11 En dat niemand deur die wet by God geregverdig word nie, is duidelik; want die regverdige sal uit die geloof lewe."},
	{12, "12 Maar die wet is nie uit die geloof nie, maar die mens wat hierdie dinge doen, sal daardeur lewe."},
	{13, "13 Christus het ons losgekoop van die vloek van die wet deur vir ons 'n vloek te word--want daar is geskrywe: Vervloek is elkeen wat aan 'n hout hang--"},
	{14, "14 sodat die seën van Abraham na die heidene kan kom in Christus Jesus, en dat ons die belofte van die Gees deur die geloof kan ontvang."},
	{15, "15 Broeders, ek spreek menslikerwys: selfs 'n mens se testament wat bekragtig is, maak niemand tot niet of voeg daaraan toe nie."},
	{16, "16 Nou is aan Abraham die beloftes toegesê en aan sy saad. Hy sê nie: En aan die sade, asof dit op baie sien nie, maar op een: En aan jou saad, dit is Christus."},
	{17, "17 Maar ek sê dít: die wet wat vier honderd en dertig jaar later gekom het, maak die verbond wat deur God in Christus vantevore bekragtig is, nie kragteloos om die belofte tot niet te maak nie."},
	{18, "18 Want as die erfenis uit die wet is, dan is dit nie meer uit die belofte nie. Maar God het dit aan Abraham deur 'n belofte genadiglik geskenk."},
	{19, "19 Wat beteken die wet dan? Dit is bygevoeg weens die oortredinge, totdat die saad aan wie die belofte gedoen is, sou kom; en dit is deur engele beskik deur tussenkoms van 'n middelaar."},
	{20, "20 En die middelaar is nie net vir een nie; maar God is een."},
	{21, "21 Is die wet dan teen die beloftes van God? Nee, stellig nie! Want as daar 'n wet gegee was wat krag het om lewend te maak, dan sou die geregtigheid werklik uit die wet wees."},
	{22, "22 Maar die Skrif het alles ingesluit onder die sonde, sodat die belofte uit die geloof in Jesus Christus aan die gelowiges gegee kon word."},
	{23, "23 Maar voordat die geloof gekom het, is ons onder die wet in bewaring gehou, ingesluit met die oog op die geloof wat geopenbaar sou word."},
	{24, "24 Die wet was dus ons tugmeester na Christus toe, sodat ons geregverdig kan word uit die geloof."},
	{25, "25 Maar nou dat die geloof gekom het, is ons nie meer onder 'n tugmeester nie."},
	{26, "26 Want julle is almal kinders van God deur die geloof in Christus Jesus;"},
	{27, "27 want julle almal wat in Christus gedoop is, het julle met Christus beklee."},
	{28, "28 Daar is nie meer Jood of Griek nie, daar is nie meer slaaf of vryman nie, daar is nie meer man en vrou nie; want julle is almal een in Christus Jesus."},
	{29, "29 En as julle aan Christus behoort, dan is julle die nageslag van Abraham en volgens die belofte erfgename."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
static void view4() {
struct af4 poems[] = {
	{1, "1 Maar ek sê: So lank as die erfgenaam 'n kind is, verskil hy niks van 'n dienskneg nie, al is hy heer van alles;"},
	{2, "2 maar hy staan onder voogde en bestuurders tot op die tyd tevore deur die vader bepaal."},
	{3, "3 So was ons ook, toe ons kinders was, in diensbaarheid onder die eerste beginsels van die wêreld."},
	{4, "4 Maar toe die volheid van die tyd gekom het, het God sy Seun uitgestuur, gebore uit 'n vrou, gebore onder die wet,"},
	{5, "5 om die wat onder die wet was, los te koop, sodat ons die aanneming tot kinders kan ontvang."},
	{6, "6 En omdat julle kinders is, het God die Gees van sy Seun in julle harte uitgestuur, en Hy roep: Abba, Vader!"},
	{7, "7 Daarom is jy nie meer dienskneg nie, maar kind; en as jy kind is, dan ook erfgenaam van God deur Christus."},
	{8, "8 Maar destyds, toe julle God nie geken het nie, het julle dié gedien wat van nature geen gode is nie;"},
	{9, "9 maar nou dat julle God ken, of liewer deur God geken is, hoe keer julle weer terug tot die swakke en armoedige eerste beginsels wat julle weer van voor af aan wil dien?"},
	{10, "10 Julle neem dae en en maande en tye en jare waar."},
	{11, "11 Ek vrees vir julle dat ek miskien tevergeefs aan julle gearbei het."},
	{12, "12 Broeders, ek bid julle, word soos ek, want ek is ook soos julle. Julle het my geen onreg aangedoen nie;"},
	{13, "13 maar julle weet dat ek ten gevolge van krankheid van die vlees die eerste keer die evangelie aan julle verkondig het;"},
	{14, "14 en julle het my beproewing in my vlees nie verag of verfoei nie, maar julle het my ontvang as 'n engel van God, ja, as Christus Jesus."},
	{15, "15 Wat was dan julle saligprysing? Want ek getuig van julle dat julle, as dit moontlik was, jul oë uitgegrawe en vir my sou gegee het."},
	{16, "16 Het ek dan julle vyand geword deur julle die waarheid te vertel?"},
	{17, "17 Hulle ywer vir julle is nie goed nie, maar hulle wil ons buitesluit, sodat julle vir hulle kan ywer."},
	{18, "18 Dit is goed as daar altyd geywer word in 'n goeie saak en nie alleen wanneer ek by julle teenwoordig is nie,"},
	{19, "19 my kinders, vir wie ek weer in barensnood is totdat Christus in julle 'n gestalte verkry."},
	{20, "20 En ek sou wens om nou by julle teenwoordig te wees en my stem te verander, want ek is buite raad met julle."},
	{21, "21 Julle wat onder die wet wil wees, sê vir my, luister julle nie na die wet nie?"},
	{22, "22 Want daar is geskrywe dat Abraham twee seuns gehad het, een uit die slavin en een uit die vrye."},
	{23, "23 Maar die een uit die slavin is gebore na die vlees, en die ander een uit die vrye deur die belofte."},
	{24, "24 Dit is sinnebeelde, want dié vroue staan vir twee verbonde: een, van die berg Sinai afkomstig, wat vir die slawerny kinders baar--dit is Hagar;"},
	{25, "25 want Hagar staan vir die berg Sinai in Arabië en kom ooreen met die teenswoordige Jerusalem en is met haar kinders saam in slawerny."},
	{26, "26 Maar Jerusalem daarbo is vry; en dit is die moeder van ons almal."},
	{27, "27 Want daar is geskrywe: Verbly jou, onvrugbare wat nie baar nie; breek uit en roep, jy wat geen barensnood het nie, want die kinders van die eensame is meer as van haar wat 'n man het."},
	{28, "28 Maar ons, broeders, is soos Isak, kinders van die belofte;"},
	{29, "29 maar soos destyds hy wat na die vlees gebore is, hom wat na die Gees was, vervolg het, so is dit nou ook."},
	{30, "30 Maar wat sê die Skrif? Werp die slavin en haar seun uit, want nooit mag die seun van die slavin saam met die seun van die vrye erwe nie."},
	{31, "31 Daarom, broeders, ons is nie kinders van die slavin nie, maar van die vrye."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
static void view5() {
struct af5 poems[] = {
	{1, "1 Staan dan vas in die vryheid waarmee Christus ons vrygemaak het, en laat julle nie weer onder die juk van diensbaarheid bring nie."},
	{2, "2 Kyk, ek, Paulus, sê vir julle dat as julle jul laat besny, Christus vir julle tot geen nut sal wees nie."},
	{3, "3 En ek betuig dit weer aan elke mens wat hom laat besny, dat hy onder verpligting is om die hele wet te onderhou."},
	{4, "4 Julle wat geregverdig wil wees deur die wet, is losgemaak van Christus; julle het van die genade verval."},
	{5, "5 Want óns verwag deur die Gees uit die geloof die geregtigheid waarop ons hoop."},
	{6, "6 Want in Christus Jesus het nòg die besnydenis nòg die onbesnedenheid enige krag, maar die geloof wat deur die liefde werk."},
	{7, "7 Julle het goed geloop: wie het julle verhinder om die waarheid gehoorsaam te wees?"},
	{8, "8 Hierdie oortuiging kom nie van Hom wat julle roep nie."},
	{9, "9 'n Bietjie suurdeeg maak die hele deeg suur."},
	{10, "10 Ek vertrou van julle in die Here dat julle nie anders sal dink nie; maar die een wat julle in die war bring, sal die oordeel dra, wie dit ook mag wees."},
	{11, "11 Maar ek, broeders, as ek nog die besnydenis verkondig, waarom word ek nog vervolg? Dan is die struikelblok van die kruis vernietig!"},
	{12, "12 Ag, as die wat julle opstandig maak, hulleself tog maar ook wou vermink!"},
	{13, "13 Want julle is tot vryheid geroep, broeders; gebruik net nie julle vryheid as 'n aanleiding vir die vlees nie, maar dien mekaar deur die liefde."},
	{14, "14 Want die hele wet word vervul in een woord, naamlik: Jy moet jou naaste liefhê soos jouself."},
	{15, "15 Maar as julle mekaar byt en opeet, pas op dat julle nie die een deur die ander verteer word nie."},
	{16, "16 Maar ek sê: Wandel deur die Gees, dan sal julle nooit die begeerlikheid van die vlees volbring nie;"},
	{17, "17 want die vlees begeer teen die Gees, en die Gees teen die vlees; en hulle staan teenoor mekaar, sodat julle nie kan doen wat julle wil nie."},
	{18, "18 Maar as julle deur die Gees gelei word, dan is julle nie onder die wet nie."},
	{19, "19 En die werke van die vlees is openbaar, naamlik owerspel, hoerery, onreinheid, ongebondenheid;"},
	{20, "20 afgodery, towery, vyandskap, twis, jaloersheid, toornigheid, naywer, tweedrag, partyskap;"},
	{21, "21 afguns, moord, dronkenskap, brassery en dergelike dinge, waarvan ek julle vooraf sê, soos ek al vroeër gesê het, dat die wat sulke dinge doen, die koninkryk van God nie sal beërwe nie."},
	{22, "22 Maar die vrug van die Gees is liefde, blydskap, vrede, lankmoedigheid, vriendelikheid, goedheid, getrouheid, sagmoedigheid, selfbeheersing."},
	{23, "23 Teen sulke dinge is die wet nie."},
	{24, "24 Maar die wat aan Christus behoort, het die vlees met sy hartstogte en begeerlikhede gekruisig."},
	{25, "25 As ons deur die Gees lewe, laat ons ook deur die Gees wandel."},
	{26, "26 Laat ons nie word soekers van ydele eer wat mekaar uittart en mekaar beny nie."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
static void view6() {
struct af6 poems[] = {
	{1, "1 Broeders, as iemand ewenwel deur een of ander misdaad oorval word, moet julle wat geestelik is, so een reghelp met die gees van sagmoedigheid, terwyl jy op jouself let, dat jy ook nie versoek word nie."},
	{2, "2 Dra mekaar se laste en vervul so die wet van Christus."},
	{3, "3 Want as iemand meen dat hy iets is, terwyl hy niks is nie, mislei hy homself."},
	{4, "4 Maar laat elkeen sy eie werk op die proef stel; en dan sal hy grond vir roem hê alleen wat homself betref en nie wat die ander betref nie."},
	{5, "5 Want elkeen sal sy eie pak dra."},
	{6, "6 Maar laat hom wat in die Woord onderwys ontvang, meedeel van alle goeie dinge aan hom wat onderwys gee."},
	{7, "7 Moenie dwaal nie; God laat Hom nie bespot nie; want net wat die mens saai, dit sal hy ook maai."},
	{8, "8 Hy wat in sy vlees saai, sal uit die vlees verderf maai; maar hy wat in die Gees saai, sal uit die Gees die ewige lewe maai."},
	{9, "9 En laat ons nie moeg word om goed te doen nie, want op die regte tyd sal ons maai as ons nie verslap nie."},
	{10, "10 Laat ons dan, terwyl ons geleentheid het, aan almal goed doen, maar die meeste aan die huisgenote van die geloof."},
	{11, "11 Kyk met watter groot letters ek met my eie hand aan julle skrywe."},
	{12, "12 Almal wat 'n mooi vertoning in die vlees wil maak, dié dwing julle om julle te laat besny net om nie ter wille van die kruis van Christus vervolg te word nie."},
	{13, "13 Want ook hulle wat besny word, hou self nie die wet nie; maar hulle wil hê dat julle besny moet word, sodat hulle in julle vlees kan roem."},
	{14, "14 Maar wat my betref, mag ek nooit roem nie, behalwe in die kruis van onse Here Jesus Christus, deur wie die wêreld vir my gekruisig is en ek vir die wêreld."},
	{15, "15 Want in Christus Jesus het nòg die besnydenis nòg die onbesnedenheid enige krag, maar 'n nuwe skepsel."},
	{16, "16 En mag daar vrede en barmhartigheid wees oor almal wat sal wandel volgens hierdie reël, en oor die Israel van God!"},
	{17, "17 Verder moet niemand my moeite aandoen nie, want ek dra die littekens van die Here Jesus in my liggaam."},
	{18, "18 Die genade van onse Here Jesus Christus sy met julle gees, broeders! Amen."},
};
size_t npoems = sizeof poems / sizeof poems[0];size_t i;for (i=0; i < npoems; ++i) {printf("%s\n", poems[i].msg);}
}
};